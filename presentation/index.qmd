...
::: {.incremental}
-   **The Challenge:** The original, linearized method requires an *a priori* estimate of the P-pool ($P^S \approx P_{Olsen} - P_{H_2O}$).
-   **The Observation:** This estimate was chemically invalid, leading to poor model fits.
-   **The Solution:** We adopted a robust non-linear regression to estimate parameters directly from the data.
:::

```{r}
#| label: fig-replicability-dynamic
#| echo: false
#| message: false
#| warning: false
#| fig-height: 7

# This single chunk controls the entire plot sequence.
...
# --- Step 1: All calculations are done upfront ---
# (This is the same calculation code as before)
library(dplyr); library(ggplot2); library(nlme); library(latex2exp)
d_linear <- d_plot %>%
  mutate(flawed_PS = Pv_Olsen.mg.L., Y1 = log(1 - (Pv.mg.L. / flawed_PS))) %>%
  filter(is.finite(Y1))
...
pred_data_flawed <- d_plot %>%
  select(uid, Site, Treatment, Repetition, Pv_Olsen.mg.L.) %>% distinct() %>%
...
# --- Step 2: Define the three plot versions ---
p_base <- ggplot() +
...
p1 <- p_base + labs(title = "Step 1: Raw Experimental Data")
p2 <- p_base +
...
# --- Step 3: Dynamically display the correct plot ---
# This uses revealjs's internal state to decide which plot to show
# .fragment(0) is the first state, .fragment(1) is the second, etc.
knitr::asis_output(
  '<
