...
pred_data <- d_plot %>%
  select(uid, Site, Treatment, Repetition, PS.x, k.x) %>%
  distinct() %>%
  crossing(t.min. = time_seq) %>%
  mutate(pred_Pv = PS.x * (1 - exp(-k.x * t.min.)))
...
ggplot(nlme.coef, aes(y=k*PS, x=Treatment, col=Site)) + geom_point() + geom_line(aes(group=Site)) + scale_y_sqrt()
