...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.3784}}
...
the marginal and conditional R² in our models strongly suggests that
these site- and year-specific variables, captured by the random effects,
were indeed the dominant factors.
...
required origin, thus violating a core assumption of the method. In
contrast, the direct application of a non-linear mixed-effects model
successfully captured the curvilinear nature of P release for nearly all
samples. This confirms that while the foundational concept of
first-order kinetics is sound, modern computational methods that avoid
data transformation provide a far more accurate and statistically valid
...
all chemical extractions. This highlights a fundamental challenge in
soil analysis and reinforces the importance of robust modeling. The use
of a non-linear \emph{mixed-effects} model is particularly advantageous
in this context, as it is designed to handle such individual variations
by estimating a common overall trend while allowing for random
deviations for each sample, thereby preventing a few anomalous data
points from unduly influencing the overall conclusions{[}Fisher (1925);
Fisher1935; Yates1964; Van Es et al. (2002){]}.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Make sure the required libraries are loaded}
\FunctionTok{library}\NormalTok{(ggplot2)}
\FunctionTok{library}\NormalTok{(dplyr)}

\CommentTok{\# Filter for the Ellighausen site and select relevant columns}
\NormalTok{ell\_data }\OtherTok{\textless{}{-}}\NormalTok{ D }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{filter}\NormalTok{(Site }\SpecialCharTok{==} \StringTok{"Ellighausen"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{select}\NormalTok{(Treatment, Rep, soil\_0\_20\_P\_CO2, soil\_0\_20\_P\_AAE10)}

\CommentTok{\# Determine the scaling factor for the secondary axis.}
\CommentTok{\# This is a common approach: divide the range of the primary variable by the range of the secondary.}
\NormalTok{scaling\_factor }\OtherTok{\textless{}{-}} \FunctionTok{max}\NormalTok{(ell\_data}\SpecialCharTok{$}\NormalTok{soil\_0\_20\_P\_AAE10, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{/} \FunctionTok{max}\NormalTok{(ell\_data}\SpecialCharTok{$}\NormalTok{soil\_0\_20\_P\_CO2, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}

\CommentTok{\# Create the plot}
\FunctionTok{ggplot}\NormalTok{(ell\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \FunctionTok{as.factor}\NormalTok{(Rep), }\AttributeTok{group =} \DecValTok{1}\NormalTok{)) }\SpecialCharTok{+}
  \CommentTok{\# Plot the first variable (P\_AAE10) {-} this will use the primary (left) y{-}axis}
  \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ soil\_0\_20\_P\_AAE10), }\AttributeTok{color =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ soil\_0\_20\_P\_AAE10), }\AttributeTok{color =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{size =} \DecValTok{4}\NormalTok{) }\SpecialCharTok{+}

  \CommentTok{\# Plot the second variable (P\_CO2) {-} transform it by the scaling factor}
  \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ soil\_0\_20\_P\_CO2 }\SpecialCharTok{*}\NormalTok{ scaling\_factor), }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ soil\_0\_20\_P\_CO2 }\SpecialCharTok{*}\NormalTok{ scaling\_factor), }\AttributeTok{color =} \StringTok{"red"}\NormalTok{, }\AttributeTok{size =} \DecValTok{4}\NormalTok{) }\SpecialCharTok{+}

  \CommentTok{\# Create the secondary y{-}axis}
  \FunctionTok{scale\_y\_continuous}\NormalTok{(}
    \CommentTok{\# Features for the primary y{-}axis}
    \AttributeTok{name =} \StringTok{"P\_AAE10 (mg/kg)"}\NormalTok{,}

    \CommentTok{\# Add the secondary y{-}axis and define its properties}
    \AttributeTok{sec.axis =} \FunctionTok{sec\_axis}\NormalTok{(}
      \CommentTok{\# The transformation here is the INVERSE of the one we applied to the data}
      \AttributeTok{trans =} \SpecialCharTok{\textasciitilde{}}\NormalTok{ . }\SpecialCharTok{/}\NormalTok{ scaling\_factor,}
      \AttributeTok{name =} \StringTok{"P\_CO2 (mg/kg)"}
\NormalTok{    )}
\NormalTok{  ) }\SpecialCharTok{+}
  \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{ Treatment, }\AttributeTok{scales =} \StringTok{"free\_y"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{title =} \StringTok{"Standard Soil P Tests at Ellighausen"}\NormalTok{,}
    \AttributeTok{x =} \StringTok{"Replicate Number"}\NormalTok{,}
    \AttributeTok{y =} \StringTok{"P Concentration (mg/kg)"} \CommentTok{\# This label is now redundant but kept for convention}
\NormalTok{  ) }\SpecialCharTok{+}
  \FunctionTok{theme\_bw}\NormalTok{() }\SpecialCharTok{+}
  \CommentTok{\# Adjust colors for the axis titles and text to match the lines}
  \FunctionTok{theme}\NormalTok{(}
    \AttributeTok{axis.title.y.left =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{color =} \StringTok{"blue"}\NormalTok{),}
    \AttributeTok{axis.text.y.left =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{color =} \StringTok{"blue"}\NormalTok{),}
...
P\_AAE10 (left axis) and P\_CO2 (right axis) for all replicates at the
Ellighausen (ELL) site, faceted by P fertilization treatment.}
