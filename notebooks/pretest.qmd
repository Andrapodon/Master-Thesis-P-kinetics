...
options(warn = -1)
# Dataset preparation
d <- read.csv(file = "./data/release-Data.csv", na.strings = c("", "n.a."))
...
p1 <- ggplot() +
  ...
  geom_line(data = pred_data, aes(x = t.min., y = pred_Pv, col = Repetition), size = 1) +
...
```{r}
ggplot(nlme.coefs, aes(y=PS  , x=Treatment, col=Site)) + geom_point() + geom_line(aes(group=Site))
ggplot(nlme.coefs, aes(y=k   , x=Treatment, col=Site)) + geom_point() + geom_line(aes(group=Site))
ggplot(nlme.coefs, aes(y=k*PS, x=Treatment, col=Site)) + geom_point() + geom_line(aes(group=Site)) + scale_y_sqrt()

fit.PS   <- lm(PS            ~ Treatment + Site, nlme.coefs)
fit.k    <- lm(k             ~ Treatment + Site, nlme.coefs)
fit.kPS  <- lm(I(sqrt(k*PS)) ~ Treatment + Site, nlme.coefs)
