...
#| fig-cap: "Annual dry matter yield across P fertilization treatments, faceted by experimental site. Data from 2017-2022."
#| echo: true
...
# Ensure your main data frame 'D' is loaded
...
# It's safest to work on a copy of your original dataframe
D_corrected <- D

# Identify the rows for Cadenazzo that are either P0 or P166
rows_to_swap <- D_corrected$Site == "Cadenazzo" & (D_corrected$Treatment == "P0" | D_corrected$Treatment == "P166")

# Use ifelse to swap the labels only for those specific rows
D_corrected$Treatment[rows_to_swap] <- ifelse(
  D_corrected$Treatment[rows_to_swap] == "P0", 
  "P166", 
  "P0"
)

# --- Verification Step ---
# Now, let's create a plot to visually inspect the change.
# We'll plot only Cadenazzo, showing both original and corrected data.

# Add a column to identify the data version
D$version <- "Original"
D_corrected$version <- "Corrected"

# Filter for Cadenazzo and combine the two dataframes
...
```{r}
#| label: fig-cadenazzo-diagnostics
#| fig-cap: "Diagnostic plots for soil properties at the Cadenazzo (CAD) site to investigate anomalies in the P0 treatment."
#| echo: true
...
# --- Plot 1: Soil Organic Carbon ---

ggplot(D %>% filter(Site == "Cadenazzo"), aes(x = Treatment, y = soil_0_20_Corg, fill = Treatment)) +
  geom_boxplot(alpha = 0.8) +
  geom_point(position = position_jitter(width = 0.1)) + # Show individual points
  labs(
    title = "Soil Organic Carbon at Cadenazzo",
    x = "P Fertilization Treatment",
    y = "Organic Carbon (g/kg)"
  ) +
  theme_bw() +
  theme(legend.position = "none")

# --- Plot 2: Standard P Tests ---

cadenazzo_long <- D %>%
  filter(Site == "Cadenazzo") %>%
  select(Treatment, Rep, soil_0_20_P_CO2, soil_0_20_P_AAE10) %>%
  pivot_longer(
    cols = c(soil_0_20_P_CO2, soil_0_20_P_AAE10),
    names_to = "Soil_Test",
    values_to = "P_Value"
  )

ggplot(cadenazzo_long, aes(x = Treatment, y = P_Value, fill = Soil_Test)) +
  geom_boxplot(alpha = 0.8) +
  facet_wrap(~ Soil_Test, scales = "free_y") +
  labs(
    title = "Standard P Tests at Cadenazzo",
    x = "P Fertilization Treatment",
    y = "P Concentration (mg/kg)"
  ) +
  theme_bw() +
  theme(legend.position = "none")
```
