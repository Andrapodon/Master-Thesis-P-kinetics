...
![Flow-chart of the adapted Kinetik-experiment after Flossmann & Richter.](/experiment-flow.png)
...
#### Variables Used in the Models {#sec-variables-used-in-the-models}

The response and predictor variables used in the linear mixed-effects models are defined in the table below.



#### Modeling of Soil and Agronomic Parameters

To analyze the relationships between the P availability metrics, soil properties, and agronomic outcomes, two distinct types of linear mixed-effects models were constructed using the `lme4` [@R-lme4] and `lmerTest` [@R-lmerTest] packages. This approach was chosen to properly account for the hierarchical and nested structure of the STYCS experimental data.
...
* **Fixed Effects Structure:** The fixed effects were identical for all models in this category and included the primary soil physical and chemical properties expected to influence P sorption and desorption: clay content, silt content, pH, organic carbon content, and dithionite-extractable iron ($Fe_d$) and aluminum ($Al_d$). The model took the form:
    `P-Metric ~ soil_0_20_clay + soil_0_20_pH_H2O + soil_0_20_Corg + soil_0_20_silt + Fed + Ald`

* **Random Effects Structure:** The random effects structure was designed to account for the nested design of the STYCS experiment. Random intercepts were included for `year`, `Site`, and the nesting of `block` within `Site` and `Treatment` within `Site`.
    * `(1|year)`: This term accounts for temporal variation, capturing baseline shifts that may occur across different years due to climatic differences.
    * `(1|Site)`: This accounts for the inherent differences between the six experimental locations.
    * `(1|Site:block)`: This term controls for spatial variability among the four experimental blocks *within* each site.
    * `(1|Site:Treatment)`: This term accounts for how the effect of the long-term P fertilization treatments might vary randomly across the different sites.

##### Comparative Models of Agronomic Outcomes

To test the central hypothesis of this thesis, the predictive power of the kinetic parameters was directly compared against that of the standard STP methods (referred to as the "GRUD" system). For each agronomic response variable (Normalized Yield, P Uptake, and P Balance), two competing models were built. These models shared an identical random effects structure to ensure a fair comparison, differing only in their fixed effects.

* **Random Effects Structure (for all agronomic models):** The random effect structure `(1|year) + (1|Site) + (1|Site:block)` was used to control for the experimental design, accounting for variations due to the growing season, location, and in-field spatial differences. This allows for the precise isolation of the fixed effects of interest.

* **Model 1: The Kinetic Approach**
    * **Fixed Effects:** This model used the kinetic parameters and their interaction: `k * log(PS)`. The interaction term is critical, as it tests the hypothesis that the agronomic benefit of a large desorbable P pool (`PS`) is dependent on the *rate* (`k`) at which that pool can be accessed by plants.
    * **Specific Models:**
        * `Ymain_norm ~ k * log(PS) + (1|year) + (1|Site) + (1|Site:block)`
        * `annual_P_uptake ~ k * log(PS) + (1|year) + (1|Site) + (1|Site:block)`
        * `annual_P_balance ~ k * log(PS) + (1|year) + (1|Site) + (1|Site:block)`

* **Model 2: The Standard STP (GRUD) Approach**
    * **Fixed Effects:** This model used the two standard Swiss soil tests and their interaction: `P_CO2 * P_AAE10`. Using both STP measures and their interaction allows the standard approach to be tested in its most comprehensive form.
    * **Specific Models:**
        * `Ymain_norm ~ P_CO2 * P_AAE10 + (1|year) + (1|Site) + (1|Site:block)`
        * `annual_P_uptake ~ P_CO2 * P_AAE10 + (1|year) + (1|Site) + (1|Site:block)`
        * `annual_P_balance ~ P_CO2 * P_AAE10 + (1|year) + (1|Site) + (1|Site:block)`

The relative performance of these two sets of models was then evaluated to determine whether the kinetic parameters provided a significant improvement in predictive power for key agronomic outcomes.
...
The analysis of phosphorus (P) desorption kinetics was based on the principles of sequential extraction established by @Flossmann1982. The original method is described below, followed by the specific protocol adapted for this study.

### Original Method of Flossmann and Richter (1982) 

The foundational method aims to characterize the P replenishment capacity of the soil. The procedure is as follows:

1.  **Removal of Soluble P**: 17.5 g of air-dried soil is shaken with 350 ml of deionized water for one hour at 25 ± 1°C. The suspension is centrifuged and the supernatant is decanted to remove the readily soluble P fraction.
...
All data processing, statistical modeling, and visualization were conducted using the R programming language (v. 4.2.2) [@R-base]. The primary packages used for the analysis were:
- `nlme` [@R-nlme] for fitting the non-linear mixed-effects models to the kinetic data.
- `lme4` [@R-lme4] and `lmerTest` [@R-lmerTest] for fitting and testing the linear mixed-effects models for agronomic and soil property analyses.
- `mlr3` [@R-mlr3] for the systematic feature selection and model validation workflow.
