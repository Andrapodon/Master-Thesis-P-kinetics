...
```{r}
tmp <- cbind(
  with_treatment = Tables_earth_treatment[["lm"]],
  without_treatment = Tables_earth_notreatment[["lm"]]
)
rownames(tmp) <- rownames(Tables_earth_treatment)
(Tables_earth_lm <- tmp)
```
...
saveRDS(list(
  ...
  table_yield_lm = Tables_yield_lm,
  table_yield_weather_lm = Tables_yield_weather_lm,
  table_earth_lm = Tables_earth_lm
