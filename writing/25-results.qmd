...
lmer_models_yield <- list(
  ...
  "m3" = lmer(Ymain_norm ~ kPS_log + (1 | Site) + (1 | year_fac) + (1 | crop), data = D, REML = F),
  "m4" = lmer(Ymain_norm ~ P_CO2_log + kPS_log + (1 | Site) + (1 | year_fac) + (1 | crop), data = D, REML = F)
)

# Define simplified labels for the table
covariate_labels_simple <- c(
  "(Intercept)" = "Intercept",
  "soil_0_20_P_CO2_log" = "$log(P_{CO_2})$",
  "soil_0_20_P_AAE10_log" = "$log(P_{AAE10})$",
  "kPS_log" = "$log(J_0)$",
  "R2m" = "Marginal R²",
  "R2c" = "Conditional R²"
)
...
model_labels_yield <- c(
  "m1" = "$Y_{norm} \\sim log(P_{CO_2})$",
  "m2" = "$Y_{norm} \\sim log(P_{AAE10})$",
  "m3" = "$Y_{norm} \\sim log(J_0)$",
  "m4" = "$Y_{norm} \\sim log(P_{CO_2}) + log(J_0)$"
)
...
lmer_models_uptake <- list(
  ...
  "m3" = lmer(annual_P_uptake ~ kPS_log + (1 | Site) + (1 | year_fac) + (1 | crop), data = D, REML = F),
  "m4" = lmer(annual_P_uptake ~ P_AAE10_log + kPS_log + (1 | Site) + (1 | year_fac) + (1 | crop), data = D, REML = F)
)
...
lmer_models_balance <- list(
  ...
  "m3" = lmer(annual_P_balance ~ PS_log + (1 | Site) + (1 | year_fac) + (1 | crop), data = D, REML = F),
  "m4" = lmer(annual_P_balance ~ P_AAE10_log + PS_log + (1 | Site) + (1 | year_fac) + (1 | crop), data = D, REML = F)
)
...
model_labels_balance <- c(
  "m1" = "$P_{bal} \\sim log(P_{CO_2})$",
  "m2" = "$P_{bal} \\sim log(P_{AAE10})$",
  "m3" = "$P_{bal} \\sim log(P_{desorb})$",
  "m4" = "$P_{bal} \\sim log(P_{AAE10}) + log(P_{desorb})$"
)
