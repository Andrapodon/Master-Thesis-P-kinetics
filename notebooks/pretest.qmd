...
ggplot(nlme.coef, aes(y=k   , x=Treatment, col=Site)) + geom_point() #+ geom_line(aes(group=Site))
ggplot(nlme.coef, aes(y=k*PS, x=Treatment, col=Site)) + geom_point() + scale_y_sqrt()#+ geom_line(aes(group=Site)) 


ggplot(nlme.coef, aes(y=PS  , x=Site, col=Treatment)) + geom_point() #+ geom_line(aes(group=Site))
ggplot(nlme.coef, aes(y=k   , x=Site, col=Treatment)) + geom_point() #+ geom_line(aes(group=Site))
ggplot(nlme.coef, aes(y=k*PS, x=Site, col=Treatment)) + geom_point() + scale_y_sqrt()#+ geom_line(aes(group=Site)) 
...
Anova(fit.PS)
#summary(glht(fit.PS, mcp(soil_0_20_clay = "Tukey")))
# Fazit: PS wird von treatment stark beeinfluss, k eher nicht (dafür von site)
Anova(fit.k)
#summary(glht(fit.k, mcp(soil_0_20_clay = "Tukey")))

Anova(fit.kPS)
#summary(glht(fit.kPS, mcp(soil_0_20_clay = "Tukey")))
Anova(fit.Yrel)
Anova(fit.Puptake)
```
