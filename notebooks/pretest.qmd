...
for (lvl in levels(d$ui)){
  ...
  model4 <- nlme(Pv.mg.L. ~ PS * (1 - exp(-k * t.dt)),
                fixed = PS + k ~ 1,
                random = PS + k ~ 1 | uid,
                data = d[, c("Pv.mg.L.", "uid", "t.dt")],
                start = c(PS = 0.05, k = 0.15),
...
nlme.coef.mrg$kPS_log <- log(nlme.coef.mrg$kPS)
nlme.coef.mrg$PS_log <- log(nlme.coef.mrg$PS)
nlme.coef.mrg$soil_0_20_P_AAE10_log <- log(nlme.coef.mrg$soil_0_20_P_AAE10)
nlme.coef.mrg$soil_0_20_P_CO2_log <- log(nlme.coef.mrg$soil_0_20_P_CO2)

nrow(nlme.coef.mrg)
names(nlme.coef.mrg)

nrow(allP[allP$year==2017,])
nrow(allP[allP$year==2018,])

RES$nlme.coef.mrg <- nlme.coef.mrg
