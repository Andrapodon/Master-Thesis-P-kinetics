...
```{r}
#| label: tbl-variables
#| tbl-cap: "Description of variables used in the agronomic models."

variables_df <- data.frame(
  Abbreviation = c("$Y_{rel}$","$Y_{norm}$", "$P_{up}$", "$P_{bal}$", "$k$", "$P_{desorb}$", "$J_0$", "$P_{CO2}$", "$P_{AAE10}$","$Al_{ox}$","$Fe_{ox}$"),
  `Full Name` = c("Relative Yield","Normalized Yield", "P Uptake", "P Balance", "Rate Constant", "Desorbable P", "Initial P Flux", "Water-Soluble P", "Chelate-Extractable P","Oxalate-extractable Al","Oxalate-extractable Fe"),
  Role = c("Response","Response", "Response", "Response", "Predictor", "Predictor", "Predictor", "Predictor", "Predictor","Predictor","Predictor"),
  Description = c(
    ...
    "Total P removed by the harvested crop biomass over a growing season (kg P ha⁻¹).",
    "Net P budget, calculated as P inputs (fertilizer) minus P outputs (uptake) (kg P ha⁻¹).",
    ...
    "Al extractable by oxalate ($mg~Fe~kg^{-1}$)",
    "Fe extractable by oxalate ($mg~Fe~kg^{-1}$)"
  )
)
...
#### Linear Mixed-Effects Model Structure {#sec-linear-mixed-effects-model-structure}
...
-   **Fixed Effects**: These represent the main explanatory variables of interest whose effects we wanted to quantify. The fixed effects included the kinetic parameters ($k$, $P_{desorb}$, $J_0$) and the standard soil P tests ($P_w$, $P_{AAE10}$), along with their interactions.
...
To identify the most informative and parsimonious model, a systematic feature selection process was conducted using the `mlr3` ([@mlr3]) machine learning framework. This involved training and evaluating different combinations of predictor variables using nested cross-validation to ensure the robustness of the final selected model.

All statistical analyses were performed in the R [@base] environment, utilizing the `nlme` [@nlme] package for kinetic modeling and the `lme4`[@lme4], `lmerTest` [@lmerTest], and `mlr3` packages for the final agronomic modeling and feature selection.
