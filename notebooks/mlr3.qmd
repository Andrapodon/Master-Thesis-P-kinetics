...
P_var_sets <- list(
  ...
  k = "k",
  PS = "PS_log",
  kPS_log = c("PS_log", "k", "kPS_log"),
  k_logPS = c("PS_log", "k", "k_logPS"),
  AAE10 = "soil_0_20_P_AAE10_log",
  CO2 = "soil_0_20_P_CO2_log",
  AAE10_CO2 = c("soil_0_20_P_AAE10_log", "soil_0_20_P_CO2_log", "soil_0_20_P_AAE10_CO2_log"),
  AAE10_CO2_k_logPS = c("soil_0_20_P_AAE10_log", "soil_0_20_P_CO2_log", "PS_log", "k", "k_logPS"),
...
```{r}
Y_vars <- c("Ymain_norm", "Ymain_rel", "annual_P_uptake", "annual_P_balance")

```

Now check NA's
```{r}
sapply(D[c(Weather_vars, unique(unlist(P_var_sets)), Y_vars)], 
  \(x) sum(is.na(x)))
...
# remove the 59 missing observations, the rest should be imputed with 
```
...
\(nam){
  mytsk <- as_task_regr(
    D[complete.cases(D$Ymain_rel), c(y, Weather_vars, P_var_sets[[nam]], "Site")],
    target = y,
    id = nam)
...
for (nam in names(P_var_sets)) {
  mytsk <- as_task_regr(
    D[complete.cases(D[,c("Ymain_rel",Weather_vars,P_var_sets$AAE10_CO2_kPS)]),
      c(y, Weather_vars, P_var_sets[[nam]], "Site")],
    target = y,
    id = nam)
  mytsk$set_col_roles("Site", "group")
  tmp[[nam]] <- mytsk
}
get_benchi_table(tmp) |> knitr::kable()

lapply(names(P_var_sets), \(nam) 
  mytsk <- as_task_regr(
    D[complete.cases(D[,c("Ymain_rel",Weather_vars,P_var_sets$AAE10_CO2_kPS)]),c(y, Weather_vars, P_var_sets[[nam]])],
    target = y,
    id = nam)
  mytsk$set_col_roles("Site", "group")
  mytsk
...
y <- "Ymain_rel"
lapply(names(P_var_sets), \(nam) as_task_regr(
    D[complete.cases(D[,c("Ymain_rel",Weather_vars,P_var_sets$AAE10_CO2_kPS)]),c(y, P_var_sets[[nam]])],
    target = y,
    id = nam)) |>
  get_benchi_table()  |> knitr::kable()
```
